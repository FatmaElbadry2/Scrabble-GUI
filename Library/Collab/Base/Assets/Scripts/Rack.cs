...
public class Rack : MonoBehaviour
{
    ...
    public static List<int> RorB = new List<int>(); //the letter on borad(0) or on rack(1)
    static int[] EmptyPlaces = new int[7];
    ...
    static public void getpos()
    {
        ...
        {
            if ( LetterList[i].transform.position.y >=0 && LetterList[i].transform.position.y <=9.1 && RorB[i] == 1)
            {
                letters.Add(LetterList[i].GetComponent<SpriteRenderer>().sprite.name);
                 col = (int)((LetterList[i].transform.position.y) / 0.65f);
                if (LetterList[i].transform.position.x > 0)
                {
                     row1 = (int)(((LetterList[i].transform.position.x) / 0.65f) + 7);
                }
                else if (LetterList[i].transform.position.x < 0)
                {
                     row1 = (int)(((LetterList[i].transform.position.x) / 0.65f) +7);
                }
                rows.Add(row1);
